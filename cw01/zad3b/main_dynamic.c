...
int main(int argc, char **argv){
    ...
    const char* cwd = get_dir();

    char *handle = calloc(256, sizeof(char));
    snprintf(handle, 256, "%s/%s", cwd, "liblibrary.so");
    void * mylib = dlopen(handle, RTLD_LAZY);
    struct main_array *(*main_array_new)(int) = (struct main_array *(*)(int)) dlsym(mylib, "main_array_new");
    void (*compare_to_tmp_file)(char **, int ) = (void (*)(char **, int)) dlsym(mylib, "compare_to_tmp_file");
    int (*save_block)(int, struct main_array *) = (int (*)(int, struct main_array *)) dlsym(mylib, "save_block");
    ...
    snprintf(path, 256, "%s/%s", cwd, "txt_files/results3bOther.txt");
    FILE *result_file;
    int nr=1;
    ...
    printf("%lf ", s);
    printf("\n");
//    append_file(result_file, "Dynamic");
