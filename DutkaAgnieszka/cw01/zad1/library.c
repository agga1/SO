...
char *get_dir_tmp() {
    char *cwd = get_dir();
    char *whole = calloc(500, sizeof(char));
    snprintf(whole, 500, "%s%s", cwd, "/tmp_files/");
    return whole;
}
char *get_dir_txt() {
    char *cwd = get_dir();
    char *whole = calloc(500, sizeof(char));
    snprintf(whole, 500, "%s%s", cwd, "/txt_files/");
    return whole;
}
...
void compare_pair_to_tmp(char *pair, char *out_filename) { // pair like "file1.txt:file2.txt"
    ...
    char command[512];
    char out_path[500];
    snprintf(out_path, sizeof(out_path), "%s%s", get_dir_tmp(), out_filename);
    snprintf(command, sizeof(command), "cd %s && diff  %s %s > %s", get_dir_txt(), file_a, file_b, out_path);
    system(command);
}
struct block* process_tmp_file(char *filename){
    ...
    char _path[500];
    snprintf(_path, sizeof(_path), "%s%s", get_dir_tmp(), filename);
